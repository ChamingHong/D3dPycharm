...
class Net(nn.Module):
    def __init__(self, upscale_factor, in_channel=1, out_channel=1, nf=64):
        ...
        self.ResBlock_layer1 = self.make_layer(functools.partial(ResBlock_3d, nf), 5)
        self.TA = nn.Conv2d(7 * nf, nf, 1, 1, bias=True)
        self.ResBlock_layer2 = self.make_layer(functools.partial(ResBlock1, nf), 6)
        self.ResBlock_layer3 = self.make_layer(functools.partial(ResBlock2, nf), 3)
        ### reconstruct
...
class ResBlock(nn.Module):
    def __init__(self, nf):
        ...
        self.dcn0 = get_pyconv(nf, 64, pyconv_kernels=[3, 5, 7, 9], pyconv_groups=[1, 4, 8, 16])
        self.dcn1 = get_pyconv(64, nf, pyconv_kernels=[3, 5, 7, 9], pyconv_groups=[1, 4, 8, 16])
        self.lrelu = nn.LeakyReLU(negative_slope=0.1, inplace=True)
